# T10: Encrypted content-addressed blob store

> **Agent contract:** 本任务是独立 Reviewer Gate。执行者只能修改 Allowed Files；完成前必须提供 RED、GREEN、全局 Gate 和原子 commit 证据。

**Wave:** W1  
**Depends on:** T09  
**Closes/advances findings:** F002, F020

## Goal

Encrypted content-addressed blob store。交付必须能被后续任务仅通过公开接口使用，不允许依赖未导出的实现细节。

## Allowed Files

- `packages/storage-node/src/blob/**`
- `packages/storage-node/test/blob-store.test.ts`

任何其他文件变化都必须先创建 `blockers/T10-scope-expansion.md`，说明必要性、接口影响和替代方案；未批准前停止。

## Produced Evidence / Interfaces

- durable encrypted CAS

```ts
export interface BlobStore { put(cursor: RuntimeCursor, plain: Uint8Array): Promise<BlobRef>; read(cursor: RuntimeCursor, ref: BlobRef, range?: ByteRange): Promise<Uint8Array>; }
```
...
```bash
python3 scripts/taskctl.py claim T10 --owner "$PCR_AGENT_ID"
mkdir -p artifacts/task-evidence/T10
git status --porcelain > artifacts/task-evidence/T10/preflight-git.txt
pnpm --version > artifacts/task-evidence/T10/preflight-toolchain.txt
```

- [ ] **Step 2 — Write the target-specific RED test**

Create `tests/tasks/t10.test.ts` with:
...
describe("T10 Encrypted content-addressed blob store", () => {
  it("encrypted_content_addressed_blob_store", async () => {
    const result = await runT10Fixture();
    expect(result).toMatchObject({ ok: true, task: "T10" });
  });
});
```
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t10.test.ts 2>&1 | tee artifacts/task-evidence/T10/red.log
```

Expected: non-zero exit and an assertion/module failure directly naming the missing T10 contract. Environment or dependency installation failures are blockers, not acceptable RED.
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t10.test.ts 2>&1 | tee artifacts/task-evidence/T10/green.log
```
...
```bash
set -o pipefail
pnpm check:all && pnpm vitest run tests/acceptance --passWithNoTests 2>&1 | tee artifacts/task-evidence/T10/full-gate.log
...
git add -- packages/storage-node/src/blob/** packages/storage-node/test/blob-store.test.ts tests/tasks/t10.test.ts artifacts/task-evidence/T10
git commit -m "feat(t10): encrypted content-addressed blob store"
