# T35: Evidence-cited semantic proposer
...
Evidence-cited semantic proposer。交付必须能被后续任务仅通过公开接口使用，不允许依赖未导出的实现细节。
...
任何其他文件变化都必须先创建 `blockers/T35-scope-expansion.md`，说明必要性、接口影响和替代方案；未批准前停止。
...
```ts
export interface SemanticProposer { propose(input:SemanticProposalInput,signal:AbortSignal):Promise<SemanticProposal>; }
```
...
```bash
python3 scripts/taskctl.py claim T35 --owner "$PCR_AGENT_ID"
mkdir -p artifacts/task-evidence/T35
git status --porcelain > artifacts/task-evidence/T35/preflight-git.txt
pnpm --version > artifacts/task-evidence/T35/preflight-toolchain.txt
```

- [ ] **Step 2 — Write the target-specific RED test**

Create `tests/tasks/t35.test.ts` with:
...
describe("T35 Evidence-cited semantic proposer", () => {
  it("evidence_cited_semantic_proposer", async () => {
    const result = await runT35Fixture();
    expect(result).toMatchObject({ ok: true, task: "T35" });
  });
});
```
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t35.test.ts 2>&1 | tee artifacts/task-evidence/T35/red.log
```

Expected: non-zero exit and an assertion/module failure directly naming the missing T35 contract. Environment or dependency installation failures are blockers, not acceptable RED.
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t35.test.ts 2>&1 | tee artifacts/task-evidence/T35/green.log
```
...
```bash
set -o pipefail
pnpm check:all && pnpm vitest run tests/acceptance --passWithNoTests 2>&1 | tee artifacts/task-evidence/T35/full-gate.log
...
git add -- packages/runtime/src/semantic/proposer.ts packages/runtime/test/semantic/proposer.test.ts tests/tasks/t35.test.ts artifacts/task-evidence/T35
git commit -m "feat(t35): evidence-cited semantic proposer"
