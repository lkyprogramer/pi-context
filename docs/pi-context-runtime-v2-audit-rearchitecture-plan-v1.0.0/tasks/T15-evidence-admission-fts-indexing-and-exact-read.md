# T15: Evidence admission, FTS indexing and exact read

> **Agent contract:** 本任务是独立 Reviewer Gate。执行者只能修改 Allowed Files；完成前必须提供 RED、GREEN、全局 Gate 和原子 commit 证据。

**Wave:** W1  
**Depends on:** T09, T10, T13, T14  
**Closes/advances findings:** F007, F020

## Goal

Evidence admission, FTS indexing and exact read。交付必须能被后续任务仅通过公开接口使用，不允许依赖未导出的实现细节。
...
任何其他文件变化都必须先创建 `blockers/T15-scope-expansion.md`，说明必要性、接口影响和替代方案；未批准前停止。

## Produced Evidence / Interfaces

- hash-verifiable evidence retrieval

```ts
export interface EvidenceService { admit(input:EvidenceAdmission):Promise<EvidenceRecord[]>; search(q:EvidenceQuery):Promise<SearchHit[]>; read(req:EvidenceRead):Promise<ExactPage>; }
```
...
```bash
python3 scripts/taskctl.py claim T15 --owner "$PCR_AGENT_ID"
mkdir -p artifacts/task-evidence/T15
git status --porcelain > artifacts/task-evidence/T15/preflight-git.txt
pnpm --version > artifacts/task-evidence/T15/preflight-toolchain.txt
```

- [ ] **Step 2 — Write the target-specific RED test**

Create `tests/tasks/t15.test.ts` with:
...
describe("T15 Evidence admission, FTS indexing and exact read", () => {
  it("evidence_admission_fts_indexing_and_exact_read", async () => {
    const result = await runT15Fixture();
    expect(result).toMatchObject({ ok: true, task: "T15" });
  });
});
```
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t15.test.ts 2>&1 | tee artifacts/task-evidence/T15/red.log
```

Expected: non-zero exit and an assertion/module failure directly naming the missing T15 contract. Environment or dependency installation failures are blockers, not acceptable RED.
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t15.test.ts 2>&1 | tee artifacts/task-evidence/T15/green.log
```
...
```bash
set -o pipefail
pnpm check:all && pnpm vitest run tests/acceptance --passWithNoTests 2>&1 | tee artifacts/task-evidence/T15/full-gate.log
...
git add -- packages/runtime/src/evidence-service.ts packages/storage-node/src/evidence-repository.ts packages/storage-node/src/fts-index.ts tests/acceptance/evidence-recovery.test.ts tests/tasks/t15.test.ts artifacts/task-evidence/T15
git commit -m "feat(t15): evidence admission, fts indexing and exact read"
