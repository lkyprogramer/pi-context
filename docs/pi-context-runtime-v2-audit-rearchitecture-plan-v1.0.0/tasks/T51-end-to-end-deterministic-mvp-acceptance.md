# T51: End-to-end deterministic MVP acceptance

> **Agent contract:** 本任务是独立 Reviewer Gate。执行者只能修改 Allowed Files；完成前必须提供 RED、GREEN、全局 Gate 和原子 commit 证据。

**Wave:** W6  
**Depends on:** T23, T28, T31, T32, T49  
**Closes/advances findings:** F001, F002

## Goal

End-to-end deterministic MVP acceptance。交付必须能被后续任务仅通过公开接口使用，不允许依赖未导出的实现细节。
...
任何其他文件变化都必须先创建 `blockers/T51-scope-expansion.md`，说明必要性、接口影响和替代方案；未批准前停止。

## Produced Evidence / Interfaces

- single release candidate verdict

```ts
export interface MvpAcceptance { vertical:boolean; recovery:boolean; w1Gate:boolean; w2Decision:string; p0Open:number; }
```
...
```bash
python3 scripts/taskctl.py check-ready T51
...
```bash
python3 scripts/taskctl.py claim T51 --owner "$PCR_AGENT_ID"
mkdir -p artifacts/task-evidence/T51
git status --porcelain > artifacts/task-evidence/T51/preflight-git.txt
pnpm --version > artifacts/task-evidence/T51/preflight-toolchain.txt
```

- [ ] **Step 2 — Write the target-specific RED test**

Create `tests/tasks/t51.test.ts` with:
...
describe("T51 End-to-end deterministic MVP acceptance", () => {
  it("end_to_end_deterministic_mvp_acceptance", async () => {
    const result = await runT51Fixture();
    expect(result).toMatchObject({ ok: true, task: "T51" });
  });
});
```
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t51.test.ts 2>&1 | tee artifacts/task-evidence/T51/red.log
```

Expected: non-zero exit and an assertion/module failure directly naming the missing T51 contract. Environment or dependency installation failures are blockers, not acceptable RED.
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t51.test.ts 2>&1 | tee artifacts/task-evidence/T51/green.log
```
...
```bash
set -o pipefail
pnpm check:all && pnpm vitest run tests/acceptance --passWithNoTests 2>&1 | tee artifacts/task-evidence/T51/full-gate.log
...
```bash
python3 scripts/taskctl.py seal-evidence T51
python3 scripts/taskctl.py verify-evidence T51
git add -- tests/acceptance/deterministic-mvp.test.ts scripts/gates/deterministic-mvp.mjs tests/tasks/t51.test.ts artifacts/task-evidence/T51
git commit -m "feat(t51): end-to-end deterministic mvp acceptance"
python3 scripts/taskctl.py record-commit T51 HEAD
```
