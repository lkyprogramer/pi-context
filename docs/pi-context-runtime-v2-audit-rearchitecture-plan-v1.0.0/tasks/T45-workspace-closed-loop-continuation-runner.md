# T45: Workspace closed-loop continuation runner

> **Agent contract:** 本任务是独立 Reviewer Gate。执行者只能修改 Allowed Files；完成前必须提供 RED、GREEN、全局 Gate 和原子 commit 证据。

**Wave:** W5  
**Depends on:** T41, T42, T43  
**Closes/advances findings:** F019, F024

## Goal

Workspace closed-loop continuation runner。交付必须能被后续任务仅通过公开接口使用，不允许依赖未导出的实现细节。
...
任何其他文件变化都必须先创建 `blockers/T45-scope-expansion.md`，说明必要性、接口影响和替代方案；未批准前停止。

## Produced Evidence / Interfaces

- environment-based success

```ts
export interface ContinuationResult { assertions:AssertionResult[]; toolCalls:ToolCallRecord[]; finalWorkspaceHash:string; success:boolean; }
```
...
```bash
python3 scripts/taskctl.py claim T45 --owner "$PCR_AGENT_ID"
mkdir -p artifacts/task-evidence/T45
git status --porcelain > artifacts/task-evidence/T45/preflight-git.txt
pnpm --version > artifacts/task-evidence/T45/preflight-toolchain.txt
```

- [ ] **Step 2 — Write the target-specific RED test**

Create `tests/tasks/t45.test.ts` with:

```ts
import { describe, expect, it } from "vitest";

describe("T45 Workspace closed-loop continuation runner", () => {
  it("scores the executor action without reading checkpoint text", async () => {
    const result = await continueFromSnapshot(caseWithDoNotMerge(), candidateArm);
    expect(result.assertions).toContainEqual({ name: "sibling-branch-not-merged", passed: true });
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t45.test.ts 2>&1 | tee artifacts/task-evidence/T45/red.log
```

Expected: non-zero exit and an assertion/module failure directly naming the missing T45 contract. Environment or dependency installation failures are blockers, not acceptable RED.
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t45.test.ts 2>&1 | tee artifacts/task-evidence/T45/green.log
```
...
```bash
set -o pipefail
pnpm check:all && pnpm vitest run tests/acceptance --passWithNoTests 2>&1 | tee artifacts/task-evidence/T45/full-gate.log
...
git add -- packages/benchmark/src/continuation/** tests/live/closed-loop.test.ts tests/tasks/t45.test.ts artifacts/task-evidence/T45
git commit -m "feat(t45): workspace closed-loop continuation runner"
