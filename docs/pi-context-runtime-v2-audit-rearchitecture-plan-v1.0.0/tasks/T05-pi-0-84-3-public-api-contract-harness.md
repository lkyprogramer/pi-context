# T05: Pi 0.84.3 public API contract harness

> **Agent contract:** 本任务是独立 Reviewer Gate。执行者只能修改 Allowed Files；完成前必须提供 RED、GREEN、全局 Gate 和原子 commit 证据。

**Wave:** W0  
**Depends on:** T03  
**Closes/advances findings:** F003, F032

## Goal

Pi 0.84.3 public API contract harness。交付必须能被后续任务仅通过公开接口使用，不允许依赖未导出的实现细节。

## Allowed Files

- `packages/pi-adapter/src/contracts/**`
- `tests/pi-contract/**`
- `compat/pi.lock.json`

任何其他文件变化都必须先创建 `blockers/T05-scope-expansion.md`，说明必要性、接口影响和替代方案；未批准前停止。

## Produced Evidence / Interfaces

- typed hook fixtures from real Pi exports
- capability probe

```ts
export interface PiCapabilities { context: true; toolResult: true; compaction: true; lifecycle: true; }
```
...
```bash
python3 scripts/taskctl.py claim T05 --owner "$PCR_AGENT_ID"
mkdir -p artifacts/task-evidence/T05
git status --porcelain > artifacts/task-evidence/T05/preflight-git.txt
pnpm --version > artifacts/task-evidence/T05/preflight-toolchain.txt
```

- [ ] **Step 2 — Write the target-specific RED test**

Create `tests/tasks/t05.test.ts` with:

```ts
import { describe, expect, it } from "vitest";

describe("T05 Pi 0.84.3 public API contract harness", () => {
  it("pi_0_84_3_public_api_contract_harness", async () => {
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t05.test.ts 2>&1 | tee artifacts/task-evidence/T05/red.log
```

Expected: non-zero exit and an assertion/module failure directly naming the missing T05 contract. Environment or dependency installation failures are blockers, not acceptable RED.
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t05.test.ts 2>&1 | tee artifacts/task-evidence/T05/green.log
```
...
```bash
set -o pipefail
pnpm check:all && pnpm vitest run tests/acceptance --passWithNoTests 2>&1 | tee artifacts/task-evidence/T05/full-gate.log
...
git add -- packages/pi-adapter/src/contracts/** tests/pi-contract/** compat/pi.lock.json tests/tasks/t05.test.ts artifacts/task-evidence/T05
git commit -m "feat(t05): pi 0.84.3 public api contract harness"
