# T36: Deterministic semantic verifier and patcher

> **Agent contract:** 本任务是独立 Reviewer Gate。执行者只能修改 Allowed Files；完成前必须提供 RED、GREEN、全局 Gate 和原子 commit 证据。

**Wave:** W4  
**Depends on:** T18, T22, T35  
**Closes/advances findings:** F005, F006

## Goal

Deterministic semantic verifier and patcher。交付必须能被后续任务仅通过公开接口使用，不允许依赖未导出的实现细节。
...
任何其他文件变化都必须先创建 `blockers/T36-scope-expansion.md`，说明必要性、接口影响和替代方案；未批准前停止。
...
```ts
export interface SemanticVerifier { verify(p:SemanticProposal,s:RuntimeSnapshot):Promise<SemanticVerification>; }
```
...
```bash
python3 scripts/taskctl.py claim T36 --owner "$PCR_AGENT_ID"
mkdir -p artifacts/task-evidence/T36
git status --porcelain > artifacts/task-evidence/T36/preflight-git.txt
pnpm --version > artifacts/task-evidence/T36/preflight-toolchain.txt
```

- [ ] **Step 2 — Write the target-specific RED test**

Create `tests/tasks/t36.test.ts` with:
...
describe("T36 Deterministic semantic verifier and patcher", () => {
  it("deterministic_semantic_verifier_and_patcher", async () => {
    const result = await runT36Fixture();
    expect(result).toMatchObject({ ok: true, task: "T36" });
  });
});
```
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t36.test.ts 2>&1 | tee artifacts/task-evidence/T36/red.log
```

Expected: non-zero exit and an assertion/module failure directly naming the missing T36 contract. Environment or dependency installation failures are blockers, not acceptable RED.
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t36.test.ts 2>&1 | tee artifacts/task-evidence/T36/green.log
```
...
```bash
set -o pipefail
pnpm check:all && pnpm vitest run tests/acceptance --passWithNoTests 2>&1 | tee artifacts/task-evidence/T36/full-gate.log
...
git add -- packages/core/src/verifier/** packages/core/test/verifier/** tests/tasks/t36.test.ts artifacts/task-evidence/T36
git commit -m "feat(t36): deterministic semantic verifier and patcher"
