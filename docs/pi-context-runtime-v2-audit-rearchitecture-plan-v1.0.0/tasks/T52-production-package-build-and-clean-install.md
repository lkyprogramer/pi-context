# T52: Production package build and clean install

> **Agent contract:** 本任务是独立 Reviewer Gate。执行者只能修改 Allowed Files；完成前必须提供 RED、GREEN、全局 Gate 和原子 commit 证据。

**Wave:** W6  
**Depends on:** T06, T51  
**Closes/advances findings:** F015, F016, F031

## Goal

Production package build and clean install。交付必须能被后续任务仅通过公开接口使用，不允许依赖未导出的实现细节。
...
任何其他文件变化都必须先创建 `blockers/T52-scope-expansion.md`，说明必要性、接口影响和替代方案；未批准前停止。

## Produced Evidence / Interfaces

- publishable self-contained package

```ts
export interface ReleasePackage { tarball:string; sha256:string; sbom:string; cleanInstallLog:string; }
```
...
```bash
python3 scripts/taskctl.py check-ready T52
...
```bash
python3 scripts/taskctl.py claim T52 --owner "$PCR_AGENT_ID"
mkdir -p artifacts/task-evidence/T52
git status --porcelain > artifacts/task-evidence/T52/preflight-git.txt
pnpm --version > artifacts/task-evidence/T52/preflight-toolchain.txt
```

- [ ] **Step 2 — Write the target-specific RED test**

Create `tests/tasks/t52.test.ts` with:

```ts
import { describe, expect, it } from "vitest";

describe("T52 Production package build and clean install", () => {
  it("loads the actual packed tarball in an empty Pi home", async () => {
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t52.test.ts 2>&1 | tee artifacts/task-evidence/T52/red.log
```

Expected: non-zero exit and an assertion/module failure directly naming the missing T52 contract. Environment or dependency installation failures are blockers, not acceptable RED.
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t52.test.ts 2>&1 | tee artifacts/task-evidence/T52/green.log
```
...
```bash
set -o pipefail
pnpm check:all && pnpm vitest run tests/acceptance --passWithNoTests 2>&1 | tee artifacts/task-evidence/T52/full-gate.log
...
```bash
python3 scripts/taskctl.py seal-evidence T52
python3 scripts/taskctl.py verify-evidence T52
git add -- apps/pi-context-runtime/** scripts/release/pack.mjs tests/release/clean-install.test.ts tests/tasks/t52.test.ts artifacts/task-evidence/T52
git commit -m "feat(t52): production package build and clean install"
python3 scripts/taskctl.py record-commit T52 HEAD
```
