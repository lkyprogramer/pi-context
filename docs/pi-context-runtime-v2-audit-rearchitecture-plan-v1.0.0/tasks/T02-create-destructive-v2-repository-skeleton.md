# T02: Create destructive v2 repository skeleton

> **Agent contract:** 本任务是独立 Reviewer Gate。执行者只能修改 Allowed Files；完成前必须提供 RED、GREEN、全局 Gate 和原子 commit 证据。

**Wave:** W0  
**Depends on:** T01  
**Closes/advances findings:** F001, F015

## Goal

Create destructive v2 repository skeleton。交付必须能被后续任务仅通过公开接口使用，不允许依赖未导出的实现细节。
...
任何其他文件变化都必须先创建 `blockers/T02-scope-expansion.md`，说明必要性、接口影响和替代方案；未批准前停止。
...
```ts
export interface PackageBoundary { from: string; allowedImports: string[]; }
```
...
```bash
python3 scripts/taskctl.py claim T02 --owner "$PCR_AGENT_ID"
mkdir -p artifacts/task-evidence/T02
git status --porcelain > artifacts/task-evidence/T02/preflight-git.txt
pnpm --version > artifacts/task-evidence/T02/preflight-toolchain.txt
```

- [ ] **Step 2 — Write the target-specific RED test**

Create `tests/tasks/t02.test.ts` with:
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t02.test.ts 2>&1 | tee artifacts/task-evidence/T02/red.log
```

Expected: non-zero exit and an assertion/module failure directly naming the missing T02 contract. Environment or dependency installation failures are blockers, not acceptable RED.
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t02.test.ts 2>&1 | tee artifacts/task-evidence/T02/green.log
```
...
```bash
set -o pipefail
pnpm check:all && pnpm vitest run tests/acceptance --passWithNoTests 2>&1 | tee artifacts/task-evidence/T02/full-gate.log
...
git add -- apps/pi-context-runtime/** packages/** pnpm-workspace.yaml tsconfig*.json vitest*.ts tests/tasks/t02.test.ts artifacts/task-evidence/T02
git commit -m "feat(t02): create destructive v2 repository skeleton"
