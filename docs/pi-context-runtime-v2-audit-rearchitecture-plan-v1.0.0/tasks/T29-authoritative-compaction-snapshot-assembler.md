# T29: Authoritative compaction snapshot assembler
...
Authoritative compaction snapshot assembler。交付必须能被后续任务仅通过公开接口使用，不允许依赖未导出的实现细节。
...
任何其他文件变化都必须先创建 `blockers/T29-scope-expansion.md`，说明必要性、接口影响和替代方案；未批准前停止。

## Produced Evidence / Interfaces

- same-transaction checkpoint source

```ts
export interface CompactionSnapshotAssembler { assemble(input:CompactionRequest):Promise<CompactionSnapshot>; }
```
...
```bash
python3 scripts/taskctl.py claim T29 --owner "$PCR_AGENT_ID"
mkdir -p artifacts/task-evidence/T29
git status --porcelain > artifacts/task-evidence/T29/preflight-git.txt
pnpm --version > artifacts/task-evidence/T29/preflight-toolchain.txt
```

- [ ] **Step 2 — Write the target-specific RED test**

Create `tests/tasks/t29.test.ts` with:
...
describe("T29 Authoritative compaction snapshot assembler", () => {
  it("authoritative_compaction_snapshot_assembler", async () => {
    const result = await runT29Fixture();
    expect(result).toMatchObject({ ok: true, task: "T29" });
  });
});
```
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t29.test.ts 2>&1 | tee artifacts/task-evidence/T29/red.log
```

Expected: non-zero exit and an assertion/module failure directly naming the missing T29 contract. Environment or dependency installation failures are blockers, not acceptable RED.
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t29.test.ts 2>&1 | tee artifacts/task-evidence/T29/green.log
```
...
```bash
set -o pipefail
pnpm check:all && pnpm vitest run tests/acceptance --passWithNoTests 2>&1 | tee artifacts/task-evidence/T29/full-gate.log
...
git add -- packages/runtime/src/compaction/snapshot.ts packages/runtime/test/compaction/snapshot.test.ts tests/tasks/t29.test.ts artifacts/task-evidence/T29
git commit -m "feat(t29): authoritative compaction snapshot assembler"
