# T30: Deterministic checkpoint renderer and verifier

> **Agent contract:** 本任务是独立 Reviewer Gate。执行者只能修改 Allowed Files；完成前必须提供 RED、GREEN、全局 Gate 和原子 commit 证据。

**Wave:** W3  
**Depends on:** T20, T29  
**Closes/advances findings:** F005, F006, F021

## Goal

Deterministic checkpoint renderer and verifier。交付必须能被后续任务仅通过公开接口使用，不允许依赖未导出的实现细节。
...
任何其他文件变化都必须先创建 `blockers/T30-scope-expansion.md`，说明必要性、接口影响和替代方案；未批准前停止。

## Produced Evidence / Interfaces

- source-backed checkpoint v2

```ts
export interface CheckpointVerifier { verify(snapshot:CompactionSnapshot,candidate:CheckpointV2):Promise<VerificationReport>; }
```
...
```bash
python3 scripts/taskctl.py claim T30 --owner "$PCR_AGENT_ID"
mkdir -p artifacts/task-evidence/T30
git status --porcelain > artifacts/task-evidence/T30/preflight-git.txt
pnpm --version > artifacts/task-evidence/T30/preflight-toolchain.txt
```

- [ ] **Step 2 — Write the target-specific RED test**

Create `tests/tasks/t30.test.ts` with:
...
describe("T30 Deterministic checkpoint renderer and verifier", () => {
  it("deterministic_checkpoint_renderer_and_verifier", async () => {
    const result = await runT30Fixture();
    expect(result).toMatchObject({ ok: true, task: "T30" });
  });
});
```
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t30.test.ts 2>&1 | tee artifacts/task-evidence/T30/red.log
```

Expected: non-zero exit and an assertion/module failure directly naming the missing T30 contract. Environment or dependency installation failures are blockers, not acceptable RED.
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t30.test.ts 2>&1 | tee artifacts/task-evidence/T30/green.log
```
...
```bash
set -o pipefail
pnpm check:all && pnpm vitest run tests/acceptance --passWithNoTests 2>&1 | tee artifacts/task-evidence/T30/full-gate.log
...
git add -- packages/core/src/compaction/** packages/core/test/compaction/** tests/tasks/t30.test.ts artifacts/task-evidence/T30
git commit -m "feat(t30): deterministic checkpoint renderer and verifier"
