# T43: W2 B0/B1/B2 live arm runner

> **Agent contract:** 本任务是独立 Reviewer Gate。执行者只能修改 Allowed Files；完成前必须提供 RED、GREEN、全局 Gate 和原子 commit 证据。

**Wave:** W5  
**Depends on:** T31, T33, T39, T40, T41  
**Closes/advances findings:** F023, F025

## Goal

W2 B0/B1/B2 live arm runner。交付必须能被后续任务仅通过公开接口使用，不允许依赖未导出的实现细节。
...
任何其他文件变化都必须先创建 `blockers/T43-scope-expansion.md`，说明必要性、接口影响和替代方案；未批准前停止。

## Produced Evidence / Interfaces

- fair compactor evaluation

```ts
export interface W2ArmRunner { run(caseId:string,arm:"B0"|"B1"|"B2",seed:number):Promise<ArmResult>; }
```
...
```bash
python3 scripts/taskctl.py claim T43 --owner "$PCR_AGENT_ID"
mkdir -p artifacts/task-evidence/T43
git status --porcelain > artifacts/task-evidence/T43/preflight-git.txt
pnpm --version > artifacts/task-evidence/T43/preflight-toolchain.txt
```

- [ ] **Step 2 — Write the target-specific RED test**

Create `tests/tasks/t43.test.ts` with:
...
describe("T43 W2 B0/B1/B2 live arm runner", () => {
  it("w2_b0_b1_b2_live_arm_runner", async () => {
    const result = await runT43Fixture();
    expect(result).toMatchObject({ ok: true, task: "T43" });
  });
});
```
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t43.test.ts 2>&1 | tee artifacts/task-evidence/T43/red.log
```

Expected: non-zero exit and an assertion/module failure directly naming the missing T43 contract. Environment or dependency installation failures are blockers, not acceptable RED.
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t43.test.ts 2>&1 | tee artifacts/task-evidence/T43/green.log
```
...
```bash
set -o pipefail
pnpm check:all && pnpm vitest run tests/acceptance --passWithNoTests 2>&1 | tee artifacts/task-evidence/T43/full-gate.log
...
git add -- packages/benchmark/src/arms/w2.ts tests/live/w2-paired.test.ts tests/tasks/t43.test.ts artifacts/task-evidence/T43
git commit -m "feat(t43): w2 b0/b1/b2 live arm runner"
