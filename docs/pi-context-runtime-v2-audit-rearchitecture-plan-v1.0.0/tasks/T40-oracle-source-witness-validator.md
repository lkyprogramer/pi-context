# T40: Oracle source-witness validator
...
Oracle source-witness validator。交付必须能被后续任务仅通过公开接口使用，不允许依赖未导出的实现细节。
...
任何其他文件变化都必须先创建 `blockers/T40-scope-expansion.md`，说明必要性、接口影响和替代方案；未批准前停止。

## Produced Evidence / Interfaces

- reject unsupported expected values
...
```bash
python3 scripts/taskctl.py claim T40 --owner "$PCR_AGENT_ID"
mkdir -p artifacts/task-evidence/T40
git status --porcelain > artifacts/task-evidence/T40/preflight-git.txt
pnpm --version > artifacts/task-evidence/T40/preflight-toolchain.txt
```

- [ ] **Step 2 — Write the target-specific RED test**

Create `tests/tasks/t40.test.ts` with:
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t40.test.ts 2>&1 | tee artifacts/task-evidence/T40/red.log
```

Expected: non-zero exit and an assertion/module failure directly naming the missing T40 contract. Environment or dependency installation failures are blockers, not acceptable RED.
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t40.test.ts 2>&1 | tee artifacts/task-evidence/T40/green.log
```
...
```bash
set -o pipefail
pnpm check:all && pnpm vitest run tests/acceptance --passWithNoTests 2>&1 | tee artifacts/task-evidence/T40/full-gate.log
...
git add -- packages/benchmark/src/oracle/** packages/benchmark/test/oracle.test.ts tests/tasks/t40.test.ts artifacts/task-evidence/T40
git commit -m "feat(t40): oracle source-witness validator"
