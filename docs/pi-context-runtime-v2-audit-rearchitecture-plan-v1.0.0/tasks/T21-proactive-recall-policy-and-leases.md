# T21: Proactive recall policy and leases
...
Proactive recall policy and leases。交付必须能被后续任务仅通过公开接口使用，不允许依赖未导出的实现细节。
...
任何其他文件变化都必须先创建 `blockers/T21-scope-expansion.md`，说明必要性、接口影响和替代方案；未批准前停止。

## Produced Evidence / Interfaces

- needed/not-needed bounded recall

```ts
export interface ProactiveRecallPolicy { decide(input:RecallDecisionInput):Promise<RecallDecision>; }
```
...
```bash
python3 scripts/taskctl.py claim T21 --owner "$PCR_AGENT_ID"
mkdir -p artifacts/task-evidence/T21
git status --porcelain > artifacts/task-evidence/T21/preflight-git.txt
pnpm --version > artifacts/task-evidence/T21/preflight-toolchain.txt
```

- [ ] **Step 2 — Write the target-specific RED test**

Create `tests/tasks/t21.test.ts` with:
...
describe("T21 Proactive recall policy and leases", () => {
  it("proactive_recall_policy_and_leases", async () => {
    const result = await runT21Fixture();
    expect(result).toMatchObject({ ok: true, task: "T21" });
  });
});
```
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t21.test.ts 2>&1 | tee artifacts/task-evidence/T21/red.log
```

Expected: non-zero exit and an assertion/module failure directly naming the missing T21 contract. Environment or dependency installation failures are blockers, not acceptable RED.
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t21.test.ts 2>&1 | tee artifacts/task-evidence/T21/green.log
```
...
```bash
set -o pipefail
pnpm check:all && pnpm vitest run tests/acceptance --passWithNoTests 2>&1 | tee artifacts/task-evidence/T21/full-gate.log
...
git add -- packages/core/src/retrieval/proactive.ts packages/runtime/src/lease-service.ts packages/core/test/retrieval/proactive.test.ts tests/tasks/t21.test.ts artifacts/task-evidence/T21
git commit -m "feat(t21): proactive recall policy and leases"
