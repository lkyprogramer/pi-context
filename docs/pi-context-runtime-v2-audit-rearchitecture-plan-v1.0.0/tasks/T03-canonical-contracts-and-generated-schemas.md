# T03: Canonical contracts and generated schemas

> **Agent contract:** 本任务是独立 Reviewer Gate。执行者只能修改 Allowed Files；完成前必须提供 RED、GREEN、全局 Gate 和原子 commit 证据。

**Wave:** W0  
**Depends on:** T02  
**Closes/advances findings:** F005, F033, F034

## Goal

Canonical contracts and generated schemas。交付必须能被后续任务仅通过公开接口使用，不允许依赖未导出的实现细节。
...
任何其他文件变化都必须先创建 `blockers/T03-scope-expansion.md`，说明必要性、接口影响和替代方案；未批准前停止。

## Produced Evidence / Interfaces

- single contract vocabulary
- schema generation
...
```bash
python3 scripts/taskctl.py claim T03 --owner "$PCR_AGENT_ID"
mkdir -p artifacts/task-evidence/T03
git status --porcelain > artifacts/task-evidence/T03/preflight-git.txt
pnpm --version > artifacts/task-evidence/T03/preflight-toolchain.txt
```

- [ ] **Step 2 — Write the target-specific RED test**

Create `tests/tasks/t03.test.ts` with:
...
describe("T03 Canonical contracts and generated schemas", () => {
  it("canonical_contracts_and_generated_schemas", async () => {
    const result = await runT03Fixture();
    expect(result).toMatchObject({ ok: true, task: "T03" });
  });
});
```
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t03.test.ts 2>&1 | tee artifacts/task-evidence/T03/red.log
```

Expected: non-zero exit and an assertion/module failure directly naming the missing T03 contract. Environment or dependency installation failures are blockers, not acceptable RED.
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t03.test.ts 2>&1 | tee artifacts/task-evidence/T03/green.log
```
...
```bash
set -o pipefail
pnpm check:all && pnpm vitest run tests/acceptance --passWithNoTests 2>&1 | tee artifacts/task-evidence/T03/full-gate.log
...
git add -- packages/contracts/** schemas/** scripts/generate-schemas.mjs tests/tasks/t03.test.ts artifacts/task-evidence/T03
git commit -m "feat(t03): canonical contracts and generated schemas"
