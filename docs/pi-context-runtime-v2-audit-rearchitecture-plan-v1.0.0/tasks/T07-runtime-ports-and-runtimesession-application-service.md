# T07: Runtime ports and RuntimeSession application service

> **Agent contract:** 本任务是独立 Reviewer Gate。执行者只能修改 Allowed Files；完成前必须提供 RED、GREEN、全局 Gate 和原子 commit 证据。

**Wave:** W1  
**Depends on:** T03, T04  
**Closes/advances findings:** F001

## Goal

Runtime ports and RuntimeSession application service。交付必须能被后续任务仅通过公开接口使用，不允许依赖未导出的实现细节。
...
任何其他文件变化都必须先创建 `blockers/T07-scope-expansion.md`，说明必要性、接口影响和替代方案；未批准前停止。

## Produced Evidence / Interfaces

- RuntimeSession interface and state machine

```ts
export interface RuntimeSession { ingestUserInput(i: UserInputEvent): Promise<UserInputReceipt>; ingestToolResult(i: ToolObservation): Promise<ProjectedToolResult>; materialize(i: MaterializationRequest): Promise<MaterializedView>; }
```
...
```bash
python3 scripts/taskctl.py claim T07 --owner "$PCR_AGENT_ID"
mkdir -p artifacts/task-evidence/T07
git status --porcelain > artifacts/task-evidence/T07/preflight-git.txt
pnpm --version > artifacts/task-evidence/T07/preflight-toolchain.txt
```

- [ ] **Step 2 — Write the target-specific RED test**

Create `tests/tasks/t07.test.ts` with:
...
describe("T07 Runtime ports and RuntimeSession application service", () => {
  it("runtime_ports_and_runtimesession_application_ser", async () => {
    const result = await runT07Fixture();
    expect(result).toMatchObject({ ok: true, task: "T07" });
  });
});
```
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t07.test.ts 2>&1 | tee artifacts/task-evidence/T07/red.log
```

Expected: non-zero exit and an assertion/module failure directly naming the missing T07 contract. Environment or dependency installation failures are blockers, not acceptable RED.
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t07.test.ts 2>&1 | tee artifacts/task-evidence/T07/green.log
```
...
```bash
set -o pipefail
pnpm check:all && pnpm vitest run tests/acceptance --passWithNoTests 2>&1 | tee artifacts/task-evidence/T07/full-gate.log
...
git add -- packages/runtime/src/ports.ts packages/runtime/src/runtime-session.ts packages/runtime/test/runtime-session.test.ts tests/tasks/t07.test.ts artifacts/task-evidence/T07
git commit -m "feat(t07): runtime ports and runtimesession application service"
