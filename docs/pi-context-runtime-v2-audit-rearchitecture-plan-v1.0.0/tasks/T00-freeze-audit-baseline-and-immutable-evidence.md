# T00: Freeze audit baseline and immutable evidence

> **Agent contract:** 本任务是独立 Reviewer Gate。执行者只能修改 Allowed Files；完成前必须提供 RED、GREEN、全局 Gate 和原子 commit 证据。

**Wave:** W0  
**Depends on:** 无；只依赖审计基线。  
**Closes/advances findings:** F029, F030

## Goal

Freeze audit baseline and immutable evidence。交付必须能被后续任务仅通过公开接口使用，不允许依赖未导出的实现细节。

## Allowed Files

- `audit/**`
- `artifacts/audit-baseline/**`

任何其他文件变化都必须先创建 `blockers/T00-scope-expansion.md`，说明必要性、接口影响和替代方案；未批准前停止。
...
```ts
export interface AuditBaseline { commit: string; tree: string; sourceDigests: Record<string,string>; }
```
...
```bash
python3 scripts/taskctl.py check-ready T00
...
```bash
python3 scripts/taskctl.py claim T00 --owner "$PCR_AGENT_ID"
mkdir -p artifacts/task-evidence/T00
git status --porcelain > artifacts/task-evidence/T00/preflight-git.txt
pnpm --version > artifacts/task-evidence/T00/preflight-toolchain.txt
```

- [ ] **Step 2 — Write the target-specific RED test**

Create `tests/tasks/t00.test.ts` with:
...
describe("T00 Freeze audit baseline and immutable evidence", () => {
  it("freeze_audit_baseline_and_immutable_evidence", async () => {
    const result = await runT00Fixture();
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t00.test.ts 2>&1 | tee artifacts/task-evidence/T00/red.log
```

Expected: non-zero exit and an assertion/module failure directly naming the missing T00 contract. Environment or dependency installation failures are blockers, not acceptable RED.
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t00.test.ts 2>&1 | tee artifacts/task-evidence/T00/green.log
```
...
```bash
set -o pipefail
pnpm check:all && pnpm vitest run tests/acceptance --passWithNoTests 2>&1 | tee artifacts/task-evidence/T00/full-gate.log
...
```bash
python3 scripts/taskctl.py seal-evidence T00
python3 scripts/taskctl.py verify-evidence T00
git add -- audit/** artifacts/audit-baseline/** tests/tasks/t00.test.ts artifacts/task-evidence/T00
git commit -m "feat(t00): freeze audit baseline and immutable evidence"
python3 scripts/taskctl.py record-commit T00 HEAD
```
