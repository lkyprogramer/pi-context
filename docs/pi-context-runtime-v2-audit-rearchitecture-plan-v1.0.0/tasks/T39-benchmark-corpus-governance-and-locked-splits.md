# T39: Benchmark corpus governance and locked splits

> **Agent contract:** 本任务是独立 Reviewer Gate。执行者只能修改 Allowed Files；完成前必须提供 RED、GREEN、全局 Gate 和原子 commit 证据。

**Wave:** W5  
**Depends on:** T01  
**Closes/advances findings:** F022, F028

## Goal

Benchmark corpus governance and locked splits。交付必须能被后续任务仅通过公开接口使用，不允许依赖未导出的实现细节。
...
任何其他文件变化都必须先创建 `blockers/T39-scope-expansion.md`，说明必要性、接口影响和替代方案；未批准前停止。

## Produced Evidence / Interfaces

- immutable train/dev/test hashes

```ts
export interface CorpusManifest { benchmarkMajor:number; trainHash:string; devHash:string; lockedTestHash:string; clusters:Record<string,string[]>; }
```
...
```bash
python3 scripts/taskctl.py claim T39 --owner "$PCR_AGENT_ID"
mkdir -p artifacts/task-evidence/T39
git status --porcelain > artifacts/task-evidence/T39/preflight-git.txt
pnpm --version > artifacts/task-evidence/T39/preflight-toolchain.txt
```

- [ ] **Step 2 — Write the target-specific RED test**

Create `tests/tasks/t39.test.ts` with:
...
describe("T39 Benchmark corpus governance and locked splits", () => {
  it("benchmark_corpus_governance_and_locked_splits", async () => {
    const result = await runT39Fixture();
    expect(result).toMatchObject({ ok: true, task: "T39" });
  });
});
```
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t39.test.ts 2>&1 | tee artifacts/task-evidence/T39/red.log
```

Expected: non-zero exit and an assertion/module failure directly naming the missing T39 contract. Environment or dependency installation failures are blockers, not acceptable RED.
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t39.test.ts 2>&1 | tee artifacts/task-evidence/T39/green.log
```
...
```bash
set -o pipefail
pnpm check:all && pnpm vitest run tests/acceptance --passWithNoTests 2>&1 | tee artifacts/task-evidence/T39/full-gate.log
...
git add -- packages/benchmark/src/corpus/** benchmarks/manifest/** scripts/benchmark-lock.mjs tests/tasks/t39.test.ts artifacts/task-evidence/T39
git commit -m "feat(t39): benchmark corpus governance and locked splits"
