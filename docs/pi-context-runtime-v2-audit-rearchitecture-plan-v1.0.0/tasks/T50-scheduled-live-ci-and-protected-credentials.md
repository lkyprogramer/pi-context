# T50: Scheduled live CI and protected credentials

> **Agent contract:** 本任务是独立 Reviewer Gate。执行者只能修改 Allowed Files；完成前必须提供 RED、GREEN、全局 Gate 和原子 commit 证据。

**Wave:** W5  
**Depends on:** T49  
**Closes/advances findings:** F030, F038

## Goal

Scheduled live CI and protected credentials。交付必须能被后续任务仅通过公开接口使用，不允许依赖未导出的实现细节。
...
任何其他文件变化都必须先创建 `blockers/T50-scope-expansion.md`，说明必要性、接口影响和替代方案；未批准前停止。

## Produced Evidence / Interfaces

- nightly 3-seed evidence

```ts
export interface LiveCiPolicy { requiredSecrets:string[]; maxConcurrency:number; artifactRetentionDays:number; }
```
...
```bash
python3 scripts/taskctl.py check-ready T50
...
```bash
python3 scripts/taskctl.py claim T50 --owner "$PCR_AGENT_ID"
mkdir -p artifacts/task-evidence/T50
git status --porcelain > artifacts/task-evidence/T50/preflight-git.txt
pnpm --version > artifacts/task-evidence/T50/preflight-toolchain.txt
```

- [ ] **Step 2 — Write the target-specific RED test**

Create `tests/tasks/t50.test.ts` with:
...
describe("T50 Scheduled live CI and protected credentials", () => {
  it("scheduled_live_ci_and_protected_credentials", async () => {
    const result = await runT50Fixture();
    expect(result).toMatchObject({ ok: true, task: "T50" });
  });
});
```
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t50.test.ts 2>&1 | tee artifacts/task-evidence/T50/red.log
```

Expected: non-zero exit and an assertion/module failure directly naming the missing T50 contract. Environment or dependency installation failures are blockers, not acceptable RED.
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t50.test.ts 2>&1 | tee artifacts/task-evidence/T50/green.log
```
...
```bash
set -o pipefail
pnpm check:all && pnpm vitest run tests/acceptance --passWithNoTests 2>&1 | tee artifacts/task-evidence/T50/full-gate.log
...
```bash
python3 scripts/taskctl.py seal-evidence T50
python3 scripts/taskctl.py verify-evidence T50
git add -- .github/workflows/live-benchmark.yml .github/workflows/nightly.yml scripts/ci/live-env.mjs tests/tasks/t50.test.ts artifacts/task-evidence/T50
git commit -m "feat(t50): scheduled live ci and protected credentials"
python3 scripts/taskctl.py record-commit T50 HEAD
```
