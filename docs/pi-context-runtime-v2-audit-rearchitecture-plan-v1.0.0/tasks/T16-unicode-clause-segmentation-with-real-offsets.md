# T16: Unicode clause segmentation with real offsets

> **Agent contract:** 本任务是独立 Reviewer Gate。执行者只能修改 Allowed Files；完成前必须提供 RED、GREEN、全局 Gate 和原子 commit 证据。

**Wave:** W2  
**Depends on:** T03, T12  
**Closes/advances findings:** F004, F033

## Goal

Unicode clause segmentation with real offsets。交付必须能被后续任务仅通过公开接口使用，不允许依赖未导出的实现细节。
...
任何其他文件变化都必须先创建 `blockers/T16-scope-expansion.md`，说明必要性、接口影响和替代方案；未批准前停止。

## Produced Evidence / Interfaces

- clause spans with utf8/utf16/codepoint ranges

```ts
export interface ClauseSpan { text:string; utf8ByteRange:Range; utf16Range:Range; codePointRange:Range; }
```
...
```bash
python3 scripts/taskctl.py claim T16 --owner "$PCR_AGENT_ID"
mkdir -p artifacts/task-evidence/T16
git status --porcelain > artifacts/task-evidence/T16/preflight-git.txt
pnpm --version > artifacts/task-evidence/T16/preflight-toolchain.txt
```

- [ ] **Step 2 — Write the target-specific RED test**

Create `tests/tasks/t16.test.ts` with:
...
describe("T16 Unicode clause segmentation with real offsets", () => {
  it("unicode_clause_segmentation_with_real_offsets", async () => {
    const result = await runT16Fixture();
    expect(result).toMatchObject({ ok: true, task: "T16" });
  });
});
```
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t16.test.ts 2>&1 | tee artifacts/task-evidence/T16/red.log
```

Expected: non-zero exit and an assertion/module failure directly naming the missing T16 contract. Environment or dependency installation failures are blockers, not acceptable RED.
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t16.test.ts 2>&1 | tee artifacts/task-evidence/T16/green.log
```
...
```bash
set -o pipefail
pnpm check:all && pnpm vitest run tests/acceptance --passWithNoTests 2>&1 | tee artifacts/task-evidence/T16/full-gate.log
...
git add -- packages/core/src/directives/segment.ts packages/core/test/directives/segment.test.ts tests/tasks/t16.test.ts artifacts/task-evidence/T16
git commit -m "feat(t16): unicode clause segmentation with real offsets"
