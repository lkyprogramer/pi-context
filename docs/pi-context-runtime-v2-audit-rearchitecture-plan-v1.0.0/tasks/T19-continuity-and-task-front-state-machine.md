# T19: Continuity and task-front state machine
...
Continuity and task-front state machine。交付必须能被后续任务仅通过公开接口使用，不允许依赖未导出的实现细节。
...
任何其他文件变化都必须先创建 `blockers/T19-scope-expansion.md`，说明必要性、接口影响和替代方案；未批准前停止。
...
```ts
export interface ContinuityService { apply(event:ContinuityEvent):Promise<ContinuityRevision>; current(cursor:RuntimeCursor):Promise<ContinuitySnapshot>; }
```
...
```bash
python3 scripts/taskctl.py claim T19 --owner "$PCR_AGENT_ID"
mkdir -p artifacts/task-evidence/T19
git status --porcelain > artifacts/task-evidence/T19/preflight-git.txt
pnpm --version > artifacts/task-evidence/T19/preflight-toolchain.txt
```

- [ ] **Step 2 — Write the target-specific RED test**

Create `tests/tasks/t19.test.ts` with:
...
describe("T19 Continuity and task-front state machine", () => {
  it("continuity_and_task_front_state_machine", async () => {
    const result = await runT19Fixture();
    expect(result).toMatchObject({ ok: true, task: "T19" });
  });
});
```
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t19.test.ts 2>&1 | tee artifacts/task-evidence/T19/red.log
```

Expected: non-zero exit and an assertion/module failure directly naming the missing T19 contract. Environment or dependency installation failures are blockers, not acceptable RED.
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t19.test.ts 2>&1 | tee artifacts/task-evidence/T19/green.log
```
...
```bash
set -o pipefail
pnpm check:all && pnpm vitest run tests/acceptance --passWithNoTests 2>&1 | tee artifacts/task-evidence/T19/full-gate.log
...
git add -- packages/core/src/continuity/** packages/runtime/src/continuity-service.ts packages/core/test/continuity/** tests/tasks/t19.test.ts artifacts/task-evidence/T19
git commit -m "feat(t19): continuity and task-front state machine"
