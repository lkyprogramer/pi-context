# T09: Per-workspace SQLite schema and migrations

> **Agent contract:** 本任务是独立 Reviewer Gate。执行者只能修改 Allowed Files；完成前必须提供 RED、GREEN、全局 Gate 和原子 commit 证据。

**Wave:** W1  
**Depends on:** T03, T04  
**Closes/advances findings:** F014

## Goal

Per-workspace SQLite schema and migrations。交付必须能被后续任务仅通过公开接口使用，不允许依赖未导出的实现细节。

## Allowed Files

- `packages/storage-node/src/schema/**`
- `packages/storage-node/src/sqlite-store.ts`
- `packages/storage-node/test/sqlite-store.test.ts`

任何其他文件变化都必须先创建 `blockers/T09-scope-expansion.md`，说明必要性、接口影响和替代方案；未批准前停止。

## Produced Evidence / Interfaces

- cursor-complete persistence

```ts
export interface EvidenceRepository { put(record: EvidenceRecord): Promise<void>; get(cursor: RuntimeCursor,id:string): Promise<EvidenceRecord|null>; }
```
...
```bash
python3 scripts/taskctl.py claim T09 --owner "$PCR_AGENT_ID"
mkdir -p artifacts/task-evidence/T09
git status --porcelain > artifacts/task-evidence/T09/preflight-git.txt
pnpm --version > artifacts/task-evidence/T09/preflight-toolchain.txt
```

- [ ] **Step 2 — Write the target-specific RED test**

Create `tests/tasks/t09.test.ts` with:
...
describe("T09 Per-workspace SQLite schema and migrations", () => {
  it("per_workspace_sqlite_schema_and_migrations", async () => {
    const result = await runT09Fixture();
    expect(result).toMatchObject({ ok: true, task: "T09" });
  });
});
```
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t09.test.ts 2>&1 | tee artifacts/task-evidence/T09/red.log
```

Expected: non-zero exit and an assertion/module failure directly naming the missing T09 contract. Environment or dependency installation failures are blockers, not acceptable RED.
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t09.test.ts 2>&1 | tee artifacts/task-evidence/T09/green.log
```
...
```bash
set -o pipefail
pnpm check:all && pnpm vitest run tests/acceptance --passWithNoTests 2>&1 | tee artifacts/task-evidence/T09/full-gate.log
...
git add -- packages/storage-node/src/schema/** packages/storage-node/src/sqlite-store.ts packages/storage-node/test/sqlite-store.test.ts tests/tasks/t09.test.ts artifacts/task-evidence/T09
git commit -m "feat(t09): per-workspace sqlite schema and migrations"
