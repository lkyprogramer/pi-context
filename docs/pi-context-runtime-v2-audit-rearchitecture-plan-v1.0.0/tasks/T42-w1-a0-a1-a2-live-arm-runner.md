# T42: W1 A0/A1/A2 live arm runner

> **Agent contract:** 本任务是独立 Reviewer Gate。执行者只能修改 Allowed Files；完成前必须提供 RED、GREEN、全局 Gate 和原子 commit 证据。

**Wave:** W5  
**Depends on:** T23, T39, T40, T41  
**Closes/advances findings:** F025, F028

## Goal

W1 A0/A1/A2 live arm runner。交付必须能被后续任务仅通过公开接口使用，不允许依赖未导出的实现细节。
...
任何其他文件变化都必须先创建 `blockers/T42-scope-expansion.md`，说明必要性、接口影响和替代方案；未批准前停止。

## Produced Evidence / Interfaces

- ingress/recall isolated evaluation

```ts
export interface W1ArmRunner { run(caseId:string,arm:"A0"|"A1"|"A2",seed:number):Promise<ArmResult>; }
```
...
```bash
python3 scripts/taskctl.py claim T42 --owner "$PCR_AGENT_ID"
mkdir -p artifacts/task-evidence/T42
git status --porcelain > artifacts/task-evidence/T42/preflight-git.txt
pnpm --version > artifacts/task-evidence/T42/preflight-toolchain.txt
```

- [ ] **Step 2 — Write the target-specific RED test**

Create `tests/tasks/t42.test.ts` with:
...
describe("T42 W1 A0/A1/A2 live arm runner", () => {
  it("w1_a0_a1_a2_live_arm_runner", async () => {
    const result = await runT42Fixture();
    expect(result).toMatchObject({ ok: true, task: "T42" });
  });
});
```
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t42.test.ts 2>&1 | tee artifacts/task-evidence/T42/red.log
```

Expected: non-zero exit and an assertion/module failure directly naming the missing T42 contract. Environment or dependency installation failures are blockers, not acceptable RED.
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t42.test.ts 2>&1 | tee artifacts/task-evidence/T42/green.log
```
...
```bash
set -o pipefail
pnpm check:all && pnpm vitest run tests/acceptance --passWithNoTests 2>&1 | tee artifacts/task-evidence/T42/full-gate.log
...
git add -- packages/benchmark/src/arms/w1.ts tests/live/w1-paired.test.ts tests/tasks/t42.test.ts artifacts/task-evidence/T42
git commit -m "feat(t42): w1 a0/a1/a2 live arm runner"
