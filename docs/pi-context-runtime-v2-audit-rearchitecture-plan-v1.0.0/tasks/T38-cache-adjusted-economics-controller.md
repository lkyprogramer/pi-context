# T38: Cache-adjusted economics controller

> **Agent contract:** 本任务是独立 Reviewer Gate。执行者只能修改 Allowed Files；完成前必须提供 RED、GREEN、全局 Gate 和原子 commit 证据。

**Wave:** W4  
**Depends on:** T25, T26, T37  
**Closes/advances findings:** F026, F027, F036

## Goal

Cache-adjusted economics controller。交付必须能被后续任务仅通过公开接口使用，不允许依赖未导出的实现细节。
...
任何其他文件变化都必须先创建 `blockers/T38-scope-expansion.md`，说明必要性、接口影响和替代方案；未批准前停止。

## Produced Evidence / Interfaces

- realized net with provider prices

```ts
export interface RealizedNet { avoidedInput:number; avoidedOverflow:number; summaryCost:number; recallCost:number; cacheRewrite:number; failureCost:number; net:number; }
```
...
```bash
python3 scripts/taskctl.py claim T38 --owner "$PCR_AGENT_ID"
mkdir -p artifacts/task-evidence/T38
git status --porcelain > artifacts/task-evidence/T38/preflight-git.txt
pnpm --version > artifacts/task-evidence/T38/preflight-toolchain.txt
```

- [ ] **Step 2 — Write the target-specific RED test**

Create `tests/tasks/t38.test.ts` with:
...
describe("T38 Cache-adjusted economics controller", () => {
  it("cache_adjusted_economics_controller", async () => {
    const result = await runT38Fixture();
    expect(result).toMatchObject({ ok: true, task: "T38" });
  });
});
```
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t38.test.ts 2>&1 | tee artifacts/task-evidence/T38/red.log
```

Expected: non-zero exit and an assertion/module failure directly naming the missing T38 contract. Environment or dependency installation failures are blockers, not acceptable RED.
...
```bash
set -o pipefail
pnpm vitest run tests/tasks/t38.test.ts 2>&1 | tee artifacts/task-evidence/T38/green.log
```
...
```bash
set -o pipefail
pnpm check:all && pnpm vitest run tests/acceptance --passWithNoTests 2>&1 | tee artifacts/task-evidence/T38/full-gate.log
...
git add -- packages/core/src/economics/** packages/runtime/src/economics-service.ts tests/performance/economics.test.ts tests/tasks/t38.test.ts artifacts/task-evidence/T38
git commit -m "feat(t38): cache-adjusted economics controller"
