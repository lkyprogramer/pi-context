...
**现状（2026-09-09）**：代码已落地。live `20260909-112407` → `decision: limited-balanced-trial`。默认 profile 仍是 observe。可执行脚本只在仓库 `eval/local/`；本包 `testing/harness/` 不再附带 runner。决策与 tarball SHA 记在仓库 `docs/iterations/native-first-v6.md`。
...
结果：**产品 = observe（默认）+ 安全的 `pctx_history`；balanced = 阈值触发的无损折叠（默认关闭；本环境为 trial，见上）**。删除 checkpoint/staging/semantic/ledger 四块。
...
E 阶段用现网 `openclaw/Qwen3.8-27B-WORK`（本环境也可用公开 OpenAI-compat URL；thinking=medium，不停 unit），12 场景 37 个 episode，三门机械判定。题目与 Pi 窗口配置在 [testing/harness](testing/harness/README.md)；跑矩阵用仓库 `eval/local/`。

## 本包提供什么 / 不提供什么

提供：审计事实、任务卡、场景矩阵、校验脚本。实现与评测成绩在仓库代码和 `artifacts/local-eval/<runId>/report.md`，不在本包内编造。E 阶段数字只能来自真实 run。
