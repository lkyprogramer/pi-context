...
if (args.resume && existsSync(manifestPath)) {
  ...
  for (const c of selected) {
    // AB/BA rotation per rep so no arm always goes first after a cold engine state
  ...
  shuffleGroups(order, seed);   // shuffle case order only, keep arm rotation inside each case/rep
  ...
  manifest = {
    ...
    configHash: null,   // filled from the first observe/balanced status.json and then frozen
    order,
  };
...
for (const ep of manifest.order) {
  ...
  // grading (E02): fill oracle from grade.sh
  if (result.status !== "blocked" && ep.caseId !== "H03") {
    ...
    // evidence probe (lost-critical-evidence): offline, from the episode's own session JSONL; null when the source line is absent.
    const sessionFile = join(epDir, "session", "session.jsonl");
    if (spec.evidence?.kind === "verbatim-quote" && existsSync(sessionFile)) {
      const parsed = parseSession(sessionFile);
      result.oracle = { ...(result.oracle ?? {}), quotedVerbatim: verbatimQuote(parsed, { linePattern: spec.evidence.linePattern, sinceMs: new Date(result.manifest?.startedAt ?? ep.startedAt ?? 0).getTime() }) };
    }
  }
  // arm identity
  if (result.status !== "blocked") {
    const status = existsSync(join(epDir, "status.json")) ? JSON.parse(readFileSync(join(epDir, "status.json"), "utf8")) : null;
    const agentDir = join(dirname(result.workdir), ".pi", "agent");
