...
const sandbox = !("no-sandbox" in args);
...
const manifest = {
  ...
  hostVersion: execFileSync("pi", ["--version"], { encoding: "utf8" }).trim(),
...
if (sandbox) {
  ...
  if (caseId !== "H03" && !args["allow-host"]) blocked("host execution of model-written code is only allowed for H03 (or --allow-host during E01 bring-up)");
