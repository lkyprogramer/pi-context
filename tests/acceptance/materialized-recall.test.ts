...
import {
  ...
  resolveProductMaterializerMode,
...
describe("product materialized recall", () => {
  it("defaults to pcr and honors PCR_EVAL_MATERIALIZER identity", () => {
    expect(resolveProductMaterializerMode({})).toBe("pcr");
    expect(resolveProductMaterializerMode({ env: { PCR_EVAL_MATERIALIZER: "identity" } })).toBe("identity");
    expect(resolveProductMaterializerMode({
      requestMode: "identity",
      optionMode: "pcr",
      env: { PCR_EVAL_MATERIALIZER: "pcr" },
    })).toBe("identity");
  });
  ...
  it("keeps identity context free of recall pages while pcr injects them when needed", async () => {
    const root = mkdtempSync(join(tmpdir(), "pcr-recall-mode-"));
    roots.push(root);
    const runtime = registerProductionUserTurnRuntime({
      on() {},
      registerTool() {},
    ...
    const request = {
      ...
      canonicalMessages: [{
        ...
        content: [{ type: "text" as const, text: "cache invalidation strategy" }],
      }],
      currentContextWindow: 200_192,
      maxOutputTokens: 16_384,
    ...
    const identity = await session.materialize({ ...request, operationId: "op-identity", materializerMode: "identity" });
    const pcr = await session.materialize({ ...request, operationId: "op-pcr", materializerMode: "pcr" });
    const identityText = identity.messages.flatMap((message) => (
      message.content.filter((block) => block.type === "text").map((block) => block.text)
    )).join("\n");
    const pcrText = pcr.messages.flatMap((message) => (
      message.content.filter((block) => block.type === "text").map((block) => block.text)
    )).join("\n");
    const identityRecall = identity.sections.find((section) => section.kind === "retrieval-page");
    expect(identityRecall === undefined || identityRecall.estimatedTokens === 0 || identityRecall.messageIds.length === 0).toBe(true);
    expect(identityText).not.toContain("error: boom");
    expect(identityText).not.toMatch(/lease ls_/u);
    expect(pcr.sections.map((section) => section.kind)).toContain("retrieval-page");
    expect(pcrText).toContain("error: boom");
    expect(pcrText).toMatch(/lease ls_/u);
    expect(pcrText).not.toBe(identityText);
    await runtime.close();
  });
});
