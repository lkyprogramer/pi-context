...
describe("search reducer", () => {
  ...
  it("keeps the same basename in different directories distinct", () => {
    const result = reduceSearchResult("src/a.ts:1:x\nlib/a.ts:1:x", { query: "x" });
    expect(result.facts).toHaveLength(2);
  });
});

describe("read and mutation reducers", () => {
  it("preserves a read range and truncation pointer", () => {
    const result = reduceReadResult("line", { path: "src/a.ts", start: 4, end: 8, truncated: true });
    expect(result.visibleText).toContain("src/a.ts 4-8");
    expect(result.visibleText).toContain("truncated");
    expect(result.facts[0]).toMatchObject({ kind: "read-range" });
  });
